...
class Word2vecVector(nn.Module):
    ...
    def __init__(self):
        ...
        # gensim.downloader.BASE_DIR = "D:/Graduate/idvx/Ant/Chart2Vec-origin/get_word_vectors/word2vec_model/gensim-data"
        # gensim.downloader.base_dir = "D:/Graduate/idvx/Ant/Chart2Vec-origin/get_word_vectors/word2vec_model/gensim-data"
    ...
    def forward(self, tokenized_text_list):
        ...
        for sentence in tokenized_text_list:
            ...
            for word in sentence:
                ...
                temp_sentence_list.append(np.array(word_vectors))
            temp_sentence_list=torch.FloatTensor(np.array(temp_sentence_list))
            temp_sentence_list=torch.max(temp_sentence_list,dim=0)[0].data
            word2vec_list.append(temp_sentence_list)

        temp_sentence_tensor=torch.tensor([item.detach().numpy() for item in word2vec_list]).to(device)
        return temp_sentence_tensor
...
class Chart2Vec(nn.Module):
    ...
    """
    def __init__(self, rep_size=100+400, hidden_dropout_prob=0.1,last_rep_size=300):
...
class SemanticEmbedding(nn.Module):
    """
        ...
        Inputs:
            ...
            `fact_pos_id`: numpy.array(), Identify the fact token location.
    """
    def __init__(self, fact_pos_num=8, hidden_size=100):
